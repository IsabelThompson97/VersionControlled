...
Ion addition is handled by the shared `ionsin.ions` file, which is appended to the tleap session after solvation. The `addions m K+ 0` command neutralizes the system first, then `addionsrand` places excess salt at random positions.
