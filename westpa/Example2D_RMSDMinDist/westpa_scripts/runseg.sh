...
ln -sv $WEST_SIM_ROOT/common_files/{2KOCFolded_NMR.prmtop,2KOCFolded_NMR.rst7} .
...
case $WEST_CURRENT_SEG_INITPOINT_TYPE in
    SEG_INITPOINT_CONTINUES)
	    ...
	    ln -sv $WEST_PARENT_DATA_REF/struct.prmtop ./struct.prmtop #36 ^^ Change to cp every 35-38 iterations, MAX 40 ln in a row
...
rmsd_data=$(tail -n +2 rmsd_toNMR.dat | awk '{print $2}')
...
paste <(echo "$rmsd_data") <(echo "$mindist_data") > $WEST_PCOORD_RETURN
paste <(echo "$rmsd_data") <(echo "$mindist_data") > checkpcoord.dat

# echo $firstLine > $WEST_PCOORD_RETURN || exit 1
# echo $lastLine >> $WEST_PCOORD_RETURN || exit 1


rm -f get_pcoord.cpptraj md.in parent.rst seg.info 2KOCFolded_NMR.rst7 2KOCFolded_NMR.prmtop # getClosestWat.py getMGW.py
